...
def generic_solve(
    interface: SolverInterface, expand_during_shake_tree: Bool = False
) -> AnyDict:
    """
    ...
    if interface.opts.show_online_optim == True:
        if interface.opts.online_optim == True:
            raise ValueError(
...
def generic_dispatch_bounds(interface, include_g: Bool, include_g_internal: Bool):
    ...
    if include_g_internal:
        all_g = vertcat(
            all_g, interface.get_all_penalties(interface.ocp, interface.ocp.g_internal)
        )
    ...
    if include_g:
        all_g = vertcat(
            all_g, interface.get_all_penalties(interface.ocp, interface.ocp.g)
        )
...
def generic_dispatch_obj_func(interface) -> CX:
    ...
    all_objectives = vertcat(
        all_objectives,
        interface.get_all_penalties(interface.ocp, interface.ocp.J_internal),
    )
    all_objectives = vertcat(
        all_objectives, interface.get_all_penalties([], interface.ocp.J)
    )

    for nlp in interface.ocp.nlp:
        all_objectives = vertcat(
            all_objectives, interface.get_all_penalties(nlp, nlp.J_internal)
        )
        all_objectives = vertcat(
            all_objectives, interface.get_all_penalties(nlp, nlp.J)
        )
...
def generic_get_all_penalties(
    interface, nlp: NonLinearProgram, penalties, scaled: Bool = True
):
    """
